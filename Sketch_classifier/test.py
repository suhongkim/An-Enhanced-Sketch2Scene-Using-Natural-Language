...
def test(args):
    ...
    with torch.no_grad():
        for i, (images, labels, _, names) in tqdm(enumerate(testloader)):
            plt.imshow((images[0].numpy().transpose(1,2,0) - np.min(images[0].numpy().transpose(1,2,0)))/(np.max(images[0].numpy().transpose(1,2,0)-np.min(images[0].numpy().transpose(1,2,0)))))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', nargs='?', type=str, default='./checkpoints/MobileNetV2_quickdraw_11_256x256_0-10-300_model.pth',
