...
class quickdrawLoader(data.Dataset):
    def __init__(self, root, split="train", version="simplified", is_transform=True,
        ...
        if self.split == 'test':
            ...
            suffix = '_{}_{}_{}.pkl'.format(self.split, train_fold_num,
                                            num_train_folds) if self.split == 'train' else '_{}_{}.pkl'.format(
                self.split, num_val)
            ...
            for i, f in enumerate(fs):
                if suffix == '.csv':
                    train_df = pd.read_csv(f, usecols=['key_id', 'drawing', 'recognized', 'word'])[
                        ['key_id', 'drawing', 'recognized', 'word']]
                    train_dict = train_df.to_dict('index')
                    num_train = (len(train_dict) - num_val) // num_train_folds
                    print('({:3d}) {:25s}: {:7d}'.format(i, os.path.basename(f).split('.')[0].split('_')[0].replace(' ',
                                                                                                                    '_'),
                                                         len(train_dict)))
                else:
                ...
                if num_train_folds > 0 and suffix == '.csv':
                    ...
                    for fold_num in range(num_train_folds):
                        ...
                        part_train_dict = {j: train_dict[key_index] for j, key_index in
                                           enumerate(selected_train_indices)}
                        part_train_pkl_filename = os.path.join(csv_path,
                                                               '{}_{}_{}_{}.pkl'.format(class_name, 'train', fold_num,
                                                                                        num_train_folds))
    ...
    def transform(self, img, lbl):
        if img.shape[0] != self.img_size[0] or img.shape[1] != self.img_size[1]:
            img = cv2.resize(img, (self.img_size[1], self.img_size[0]),
                             interpolation=cv2.INTER_LINEAR)  # cv2.resize shape: (W, H)
    ...
    def decode_drawing(self, raw_strokes, line_thickness=5, time_color=True, part_color=True, num_channels=3):
        ...
        for t, stroke in enumerate(raw_strokes):
            ...
            for i in range(len(stroke[0]) - 1):
                ...
                cv2.line(img, (stroke[0][i], stroke[1][i]), (stroke[0][i + 1], stroke[1][i + 1]), color, line_thickness,
                         cv2.LINE_AA)
        return img
